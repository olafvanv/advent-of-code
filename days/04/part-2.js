...
lib.getInput('4').then(input => {
  ...
  let answer = findRemovableCells(grid, 0);
  
  console.log("answer:", answer);
});

function findRemovableCells(grid, totalRemoved) {
  let removedCells = []
  let total = totalRemoved;
  ...
  for (const [rowIndex, row] of grid.entries()) {
    for (const [colIndex, col] of row.entries()) {
      ...
      if (adjacents.filter(f => f === '@').length < 4) {
        total++;
        removedCells.push([rowIndex, colIndex]);
      }
    }
  }

  removedCells.forEach(([x, y]) => grid[x][y] = ".")

  if(total > totalRemoved) {
    return findRemovableCells(grid, total);
  }
